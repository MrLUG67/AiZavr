...
import { defaultTaggingPrompt, resolveSystemPrompt } from './prompts';
import { ct, settingsIntro } from './i18n';
import { t } from '../../i18n';

export const PLUGIN_ID = 'tagger';
...
// ---------------------------------------------------------------------------
// Декларативная форма настроек (D-096). Вкладки «Модель»/«Инструкция», радио
// провайдера, выпадашка модели, многострочный промпт. Состояние полей держит
// хост-модалка; плагин получает значения одним сообщением по «Применить».
// ---------------------------------------------------------------------------
  ...
  const promptTab: ControlNode = {
    ...
    children: [
      ...
      },
      {
        ...
        disabled: config.systemPrompt.trim() === defaultTaggingPrompt().trim(),
